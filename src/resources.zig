...
pub fn getGrammars(allocator: std.mem.Allocator, path: []const u8, list: *std.ArrayList(GrammarInfo) ) !void{
...
test "get grammars" {
    ...
    defer list.deinit();
    try getGrammars(allocator, "./data/grammars", &list);
}
...
pub fn getThemes(allocator: std.mem.Allocator, path: []const u8, list: *std.ArrayList(ThemeInfo) ) !void{
    const dir = std.fs.cwd().openDir(path, .{ .iterate = true }) catch unreachable;
    var walker = dir.walk(allocator) catch unreachable;
    defer walker.deinit();
...
test "get themes" {
    ...
    defer list.deinit();
    try getThemes(allocator, "./data/themes", &list);
}
